...
func TestK8gbBasicRoundRobinExample(t *testing.T) {
	...
	t.Run("round-robin on two concurrent clusters with podinfo running", func(t *testing.T) {
		...
		assert.Equal(t, len(resolvedIPs5053), len(resolvedIPs5053))
	...
	t.Run("kill podinfo on the second cluster", func(t *testing.T) {
		...
		resolvedIPs5053,err := WaitForLocalGSLB(t, host, 5053, []string{})
		require.NoError(t, err)
		resolvedIPs5054,err := WaitForLocalGSLB(t, host, 5054, []string{})
		require.NoError(t, err)
		assert.Empty(t, resolvedIPs5053)
		assert.Empty(t, resolvedIPs5054)
	})
